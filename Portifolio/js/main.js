...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================================================
  // 2. CAPITAL RANDOM LETTERS NAME SCRAMBLER (26 A-Z Uppercase Alphabet)
  //
  // Powered by CSS Grid Ghost Overlay pattern:
  // - .scramble-ghost reserves the exact final dimensions
  // - .scramble-active sits directly on top in the exact same grid space
  // - Strictly cycles through 26 capital letters (A-Z) with zero layout shift
  // ==========================================================================

  const scrambleChars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';

  function buildScrambleSpans(el) {
    ...
    const wrappedLines = lines.map((line) => {
      ...
      return [...text].map((ch) => {
        if (ch === ' ') return `<span class="sc-char sc-space"> </span>`;
        return `<span class="sc-char" data-ch="${ch}">${ch}</span>`;
      }).join('');
    });
  ...
  function scrambleName(activeEl) {
    const charSpans = Array.from(activeEl.querySelectorAll('.sc-char:not(.sc-space)'));
    const total = charSpans.length;
    if (total === 0) return;

    document.body.classList.add('is-scrambling');

    const duration = 1000; // 1 second animation on page refresh
    const startTime = performance.now();

    function tick(now) {
      const elapsed = now - startTime;
      const globalRatio = Math.min(elapsed / duration, 1);

      charSpans.forEach((span, i) => {
        // Staggered progressive locking from left to right
        const lockThreshold = (i / total) * 0.5;
        const locked = globalRatio > lockThreshold + 0.45;

        if (locked) {
          span.textContent = span.dataset.ch;
          span.style.color = '';
        } else {
          // Strictly random capital letter from 26 A-Z characters in Green Color
          span.textContent = scrambleChars.charAt(Math.floor(Math.random() * 26));
  ...
  // Initialize scrambler: triggers ONLY on page refresh / page load (1 second duration)
  document.querySelectorAll('.scramble-wrapper, .scramble-text').forEach((wrapper) => {
    const activeLayer = wrapper.querySelector('.scramble-active') || wrapper;
    buildScrambleSpans(activeLayer);

    // Run strictly on page load/refresh with a short initial delay
    setTimeout(() => {
      scrambleName(activeLayer);
    }, 350);
  });
